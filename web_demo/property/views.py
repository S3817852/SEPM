...
def house_page(request):
    ...
    just_a_rented_list = []
    for i in list_house_id:
        just_a_empty_list.append(Room.objects.filter(house_id = i).filter(is_rented = False))
        just_a_rented_list.append(Room.objects.filter(house_id = i).filter(is_rented = True))
    
    print(just_a_empty_list)
    print(just_a_rented_list)
